...
public sealed class QuickMath : MiniGame
{
    ...
    public QuickMath(IReadOnlyList<string> players, DateTimeOffset now, IReadOnlyList<MathProblem> problems) : base("math", players, now, Math.Max(20, problems.Count * 20)) { this.problems = problems.ToArray(); questionStartedAt = StartsAt; foreach (var p in players) scores[p] = 0; }
    protected override void Update(DateTimeOffset now) { if (now - questionStartedAt >= TimeSpan.FromSeconds(20)) Advance(now); }
