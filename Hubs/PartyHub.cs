...
public sealed class PartyHub(RoomService rooms, GameManager games, RoomBroadcaster broadcaster, RoomPersistence persistence) : Hub
{
    ...
    public async Task SkipGame()
    {
        var (room, _) = rooms.Membership(Context.ConnectionId, true);
        games.Lobby(room);
        games.Start(room, null, true, room.Settings);
        await persistence.SaveAsync(room);
        await broadcaster.Publish(room);
    }
    ...
    public async Task EndResults()
    {
        var (room, _) = rooms.Membership(Context.ConnectionId, true);
        games.EndResults(room);
        await persistence.SaveAsync(room);
        await broadcaster.Publish(room);
    }
    public async Task SkipMath()
    {
        var (room, _) = rooms.Membership(Context.ConnectionId, true);
        games.SkipMath(room); await persistence.SaveAsync(room); await broadcaster.Publish(room);
    }
